...
Spyutils::spyQuaternion
Spyutils::spyQuaternion::operator*(spyQuaternion const& q) {
...
void Spyutils::spyQuaternion::rotate(float angle, Spyutils::Vector3 axis) {
	Ogre::Quaternion ogreQuaternion(
	    Ogre::Radian(Spyutils::Math::toRadians(angle)),
	    {axis.x, axis.y, axis.z});
	w = w * ogreQuaternion.w - x * ogreQuaternion.x - y * ogreQuaternion.y -
	    z * ogreQuaternion.z;
	x = w * ogreQuaternion.x + x * ogreQuaternion.w + y * ogreQuaternion.z -
	    z * ogreQuaternion.y;
	y = w * ogreQuaternion.y - x * ogreQuaternion.z + y * ogreQuaternion.w +
	    z * ogreQuaternion.x;
	z = w * ogreQuaternion.z + x * ogreQuaternion.y - y * ogreQuaternion.x +
	    z * ogreQuaternion.w;
}
...
void Spyutils::spyQuaternion::lookAt(Spyutils::Vector3 target,
                                     Spyutils::Vector3 position) {

	// Calcular la dirección en la que se debe mirar
	Spyutils::Vector3 vec = target - position;
	vec.normalize();
	Ogre::Vector3 worldY = Ogre::Vector3::UNIT_Y;
	auto xAxis = worldY.crossProduct({vec.x,vec.y,vec.z});
	xAxis.normalise();
	Ogre::Vector3 v = {vec.x, vec.y, vec.z};
	auto yAxis = v.crossProduct(xAxis);
	yAxis.normalise();
	Ogre::Quaternion quat(xAxis, yAxis, v);

	// Convertir la matriz de rotación a un quaternion
	*this = Spyutils::spyQuaternion(quat.w, quat.x, quat.y, quat.z);
}
